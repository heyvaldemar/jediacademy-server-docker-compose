...
#
# The first check here ended in `|| pgrep -f jampded`, and that branch
# could only ever succeed: `pgrep -f` searches whole command lines, and the
# shell running the check carries the pattern in its own. Measured in the
# real container, `pgrep -f ZZZ_no_such_process` exits 0. So the branch
# reached only when the game was gone was the branch that always passed.
#
...
EXACT='grep -qsx linuxjampded /proc/[0-9]*/comm'
LOOSE='pgrep -f jampded >/dev/null'
...
# named linuxjampded is not an applet it knows and exits immediately. A real standalone
# binary takes the name it is invoked under, which is what /proc/N/comm reads.
docker run -d --name "$RUN" ubuntu:24.04 sh -c '
  g=linuxj; g="${g}ampded"
  cp /bin/sleep "/tmp/$g"
  cp /bin/sleep /tmp/start_server.sh
  "/tmp/$g" 3600 &
  /tmp/start_server.sh 3600 &
...
docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do
  [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/linuxjampded" ] && kill -9 "${p##*/}" 2>/dev/null
...
for _ in $(seq 1 20); do
  n="$(docker exec "$RUN" sh -c 'c=0; for p in /proc/[0-9]*; do
        [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/linuxjampded" ] && c=$((c+1)); done; echo "$c"' 2>/dev/null)"
...
if docker exec "$RUN" sh -c 'for p in /proc/[0-9]*; do [ "$(tr "\0" "\n" < "$p/cmdline" 2>/dev/null | head -1)" = "/tmp/start_server.sh" ] && exit 0; done; exit 1' 2>/dev/null; then
